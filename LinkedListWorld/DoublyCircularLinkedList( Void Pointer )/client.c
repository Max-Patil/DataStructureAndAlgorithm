...
p_dcll_t p_bank_client_list = create_dcll_list(); 
srand(time(NULL));

//Adding data to back of the list
for(int client_id = 5; client_id < 8; client_id++)
    dcll_insert_back(p_bank_client_list, (data_t)p_clients[client_id]);
dcll_show(p_bank_client_list, display_client_info, "After pushing 3 client of Saint Petersburg to back of database(doubly circular linked list) : ");

//Adding data to front of the list
for(int client_id = 0; client_id < 5; client_id++)
    dcll_insert_front(p_bank_client_list, (data_t)p_clients[client_id]);
dcll_show(p_bank_client_list, display_client_info, "After pushing 5 client of Texas to front of database(doubly circular linked list) : ");

//Adding data to specified index of the list
dcll_insert_at(p_bank_client_list, (data_t)p_clients[8], 0);
dcll_insert_at(p_bank_client_list, (data_t)p_clients[9], 0);
dcll_insert_at(p_bank_client_list, (data_t)p_clients[10], get_dcll_size(p_bank_client_list));
dcll_insert_at(p_bank_client_list, (data_t)p_clients[11], get_dcll_size(p_bank_client_list));
dcll_insert_at(p_bank_client_list, (data_t)p_clients[12], 7);
dcll_show(p_bank_client_list, display_client_info, "After pushing 5 client of Belgium to database(doubly circular linked list) at specified index : ");

//Adding data before exisiting data of the list
dcll_insert_before(p_bank_client_list, (data_t)p_clients[13], (data_t)p_clients[9], compare_client_info);
dcll_insert_before(p_bank_client_list, (data_t)p_clients[14], (data_t)p_clients[9], compare_client_info);
dcll_show(p_bank_client_list, display_client_info, "After pushing 2 client of India before existing client of database(doubly circular linked list) : ");

//Adding data after existing data of the list
dcll_insert_after(p_bank_client_list, p_clients[15], get_dcll_tail(p_bank_client_list), compare_client_info);
dcll_insert_after(p_bank_client_list, p_clients[16], get_dcll_tail(p_bank_client_list), compare_client_info);
dcll_insert_after(p_bank_client_list, p_clients[17], get_dcll_head(p_bank_client_list), compare_client_info);
dcll_show(p_bank_client_list, display_client_info, "After pushing 3 client of India after existing client of database(doubly circular linked list) : ");

//Removing data from front of the list
puts("Removing 4 client from the front of database(doubly circular linked list) : ");
for(int le = 0; le < 4; le++)
    display_client_info((data_t)dcll_remove_front(p_bank_client_list)); NEW_LINE
dcll_show(p_bank_client_list, display_client_info, "\nAfter removing above 4 client from the front of database(doubly circular linked list) : ");


//Removing data from back of the list
puts("Removing 4 client from the back of the database(doubly circular linked list) : ");
for(int le = 0; le < 4; le++)
    display_client_info((data_t)dcll_remove_back(p_bank_client_list)); NEW_LINE
dcll_show(p_bank_client_list, display_client_info, "After removing 4 client from the back of database(doubly circular linked list) : ");

//Removing data of specified index of the list
puts("Removing 4 client at specified index of the database(doubly circular linked list)");
for(int index = 0; index < 4; index++)
    display_client_info(dcll_remove_at(p_bank_client_list, index)); NEW_LINE
dcll_show(p_bank_client_list, (data_t)display_client_info, "\nAfter removing above 4 client from the database(doubly circular linked list) of specified index : ");

//Destroying Simple database that is based on linked list and its simple operation
destroy_dcll(&p_bank_client_list, delete_client_info);
return EXIT_SUCCESS;
}
...
{   
    ...
    printf("-->Deleting : %15s client from the database(doubly circular linked list)\n", p_client->credit_info->name);
